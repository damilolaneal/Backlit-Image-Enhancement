...
def exposednessWM3(Im, u, std):
    ...
    try:
        for k in range(Im.shape[2]):
            w[:,:,k] = np.exp(-(( (Im[:,:,k]/(L-1)) - u[k])**2) / (2*(std[k]**2)))
...
def fusion3(ImA, ImB, ImC, l):
    ...
    B = np.copy(A)
    C = np.copy(A)
    dummy = np.zeros([int(np.ceil(x/2**l)*2**l), int(np.ceil(y/2**l)*2**l), 3])
            
    A[:x,:y] = np.copy(ImA)  #Padding ImA
    B[:x,:y] = np.copy(ImB)  #Padding ImB
    C[:x,:y] = np.copy(ImC)  #Padding ImB
    
    w = np.zeros((A.shape[0], A.shape[1], 2))  #Preallocation
    dummy[:,:,0] = np.copy(A)
    dummy[:,:,1] = np.copy(B)
    dummy[:,:,2] = np.copy(C)
    w = exposednessWM3(dummy, [0.45, 0.5, 0.55], [0.25, 0.25, 0.25])  #Exposedness weight map
    w0 = np.copy(w[:,:,0])  #Exposedness weight map for ImA
    w1 = np.copy(w[:,:,1])  #Exposedness weight map for ImB
    w2 = np.copy(w[:,:,1])  #Exposedness weight map for ImB
    ...
    for i in range(l-1,0,-1):
        GE = cv.pyrUp(gpC[i])
        L = cv.subtract(gpC[i-1],GE)
        lpC.append(L)
    
    # generate Gaussian pyramid for A's weight map
    G = w0.copy()
    gpW0 = [G]
    for i in range(l):
        G = cv.pyrDown(G)
    ...
    # generate Gaussian pyramid for B's weight map
    G = w1.copy()
    gpW1 = [G]
    for i in range(l):
        G = cv.pyrDown(G)
    ...
    # generate Gaussian pyramid for C's weight map
    G = w2.copy()
    gpW2 = [G]
    for i in range(l):
        G = cv.pyrDown(G)
    ...
    for la,lb, lc in zip(lpA,lpB,lpC):
        i += 1
        rows,cols = la.shape
        ls = gpW0[(l-1)-i]*la + gpW1[(l-1)-i]*lb + gpW2[(l-1)-i]*lc
        LS.append(ls)
