...
2- The naive blending in [1] is replaced with image fusion from [3], which
   uses gaussain and laplacian pyramids to fuse the images.

NOTE: Using image fusion actually yields worse results, but fortunately, CLAHE
...
def mAk_CLAHE(Im):
    ...
    #The below applies CLAHE to the image instead of global HE
    ...
    ImEnhanced = (1-alpha)*ImGamma + alpha*ImCLAHE
    ...
    # plt.figure(dpi=300)
    # plt.axis('off')
    # plt.title("y={} CLAHE(c={})".format(np.round(gamma, 2), c))
