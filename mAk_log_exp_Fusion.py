...
def mAk_log_exp_Fusion(Im):
    ...
    # (alpha, gamma) = (0.5, 2)  #Those values are from [1]
    log10 = lambda I, a: np.log10(a*(I + 1))
    exp = lambda I, y: 255 - (255 - Im)**y
    h = hist(Im)
    N = np.sum(h)  #Number of pixels in the image
    n = np.sum(h[:50])  #Number of pixels whose value are less than 50
    gamma = (N - n) / N  #The method to find gamma
    I1 = log10(Im, 0.5)  #a=0.5 in reference [3]
    I2 = exp(Im, gamma)
    # gamma = 2.2  #The value of gamma = 2.2 in [1]
    ImGamma = I1
    ImSharp = I2
    # ImGamma = gammaCorr(Im, gamma)  #Applies gamma correction to the image
    # ImSharp = np.array(Image.open('Images/r1CLAHE.tif'), dtype=float)
    # ImSharp = histEq(Im)  #Find the histogram equalized image
